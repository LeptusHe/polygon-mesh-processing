...
#include <imgui.h>
#include <cmath>
#include <simplification/ModVertexColorQuadricT.h>
...
int main(int argc, char *argv[])
{
    ...
    if (!mesh.has_vertex_colors()) {
        std::cout << "mesh does not have vertex color" << std::endl;
    }
    ...
    ModVertexColorQuadricT<Mesh>::Handle modVertexColorQuadric;
    decimater.add(modVertexColorQuadric);
    decimater.module(modVertexColorQuadric).set_max_err(100, false);

    //decimater.add(modNormalFlipping);
    //decimater.module(modNormalFlipping).set_max_normal_deviation(5.0f);

    int n_collpase = 19000;
    if (argc > 2) {
        n_collpase = std::atoi(argv[2]);
    }


    decimater.initialize();
    int count = decimater.decimate(n_collpase);
...
bool flip_normal(Mesh& mesh, Mesh::VertexHandle& vh, const Mesh::Point& p)
{
    auto p0 = mesh.point(vh);
    for (const auto fh : mesh.vf_range(vh)) {
        Mesh::HalfedgeHandle e;
